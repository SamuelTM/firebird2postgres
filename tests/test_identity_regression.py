...
class TestIdentityUnitRegression(unittest.TestCase):
    ...
    def test_identity_on_integer_domain_uses_base_type(self):
        # PostgreSQL rejects GENERATED AS IDENTITY on a domain reference even
        # when the domain is integer-based, so the DDL must resolve to the
        # bare base type while keeping IDENTITY and NOT NULL.
        t = Table('TAB_DOM_ID')
        t.columns.append(
            Column('ID', 'INTEGER', nullable=False, domain_name='d_inteiro',
                   identity_type='BY DEFAULT', identity_increment=1, identity_current=5)
        )
        ddl = t.get_create_table_query()
        self.assertIn('"id" INTEGER GENERATED BY DEFAULT AS IDENTITY NOT NULL', ddl)
        self.assertNotIn('public.', ddl)

    def test_identity_on_non_integer_domain_raises_with_context(self):
        t = Table('TAB_DOM_NUM')
        t.columns.append(
            Column('ID', 'NUMERIC(10, 2)', nullable=False, domain_name='d_valor',
                   identity_type='BY DEFAULT', identity_increment=1, identity_current=0)
        )
        with self.assertRaises(TypeError) as ctx:
            t.get_create_table_query()
        self.assertIn('TAB_DOM_NUM', str(ctx.exception))
        self.assertIn('ID', str(ctx.exception))

    def test_identity_on_non_integer_type_raises_with_context(self):
        t = Table('TAB_TXT_ID')
        t.columns.append(
        ...
        with self.assertRaises(TypeError) as ctx:
            t.get_create_table_query()
        self.assertIn('TAB_TXT_ID', str(ctx.exception))
        self.assertIn('smallint, integer or bigint', str(ctx.exception))
...
class TestIdentityLivePostgresRegression(unittest.TestCase):
    ...
    def test_live_identity_on_integer_domain_executes(self):
        # Legacy shape: identity column declared over an integer domain.
        # GENERATED AS IDENTITY on the domain reference is rejected by
        # PostgreSQL, so the DDL must resolve to the bare base type and
        # still generate values (tearDown rolls everything back).
        self.pg_cur.execute("DROP DOMAIN IF EXISTS reg_dom_inteiro CASCADE;")
        self.pg_cur.execute("CREATE DOMAIN reg_dom_inteiro AS INTEGER;")

        t = Table('t_live_dom_id')
        t.columns.append(Column('id', 'INTEGER', nullable=False, domain_name='reg_dom_inteiro',
                                identity_type='BY DEFAULT', identity_increment=1, identity_current=0))
        t.columns.append(Column('val', 'TEXT', nullable=True))
