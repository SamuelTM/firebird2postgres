...
class TestSequenceModel(unittest.TestCase):
    def test_sequence_creation_queries(self):
        ...
        seq_desc = Sequence('G', current_value=100, increment=-2)
        self.assertEqual(
            seq_desc.get_create_sequence_query(),
            'CREATE SEQUENCE "g" INCREMENT BY -2 MAXVALUE 9223372036854775807 START WITH 98;'
        )
    ...
    def test_table_with_identity_column(self):
        ...
        # Identity column with custom increment and descending options
        table2 = Table('CUSTOM_ID')
        table2.columns.append(Column('ID', 'BIGINT', nullable=False, identity_type='ALWAYS', identity_increment=5))
        table2.columns.append(Column('DESC_ID', 'BIGINT', nullable=False, identity_type='BY DEFAULT', identity_increment=-2))
        sql2 = table2.get_create_table_query()
        self.assertIn('GENERATED ALWAYS AS IDENTITY (INCREMENT BY 5)', sql2)
        self.assertIn('GENERATED BY DEFAULT AS IDENTITY (INCREMENT BY -2 MAXVALUE 9223372036854775807)', sql2)
