...
def check_plpgsql_runtime_validity(
    ...
    filter_names = set(n.strip('"').lower() for n in function_names) if function_names is not None else None
    ...
    for fn_oid, fn_name, nsp, identity_args in regular_funcs:
        ...
        if filter_names is None:
            ...
            for fn in filter_names:
                if (fn == fn_name.lower() or 
                    fn == full_sig.lower() or 
                    full_sig.lower().startswith(f"{fn}(") or
                    fn.startswith(f"{fn_name.lower()}(")):
                    matched_filter = True
                    verified_targets.add(fn)
        if matched_filter:
            check_targets.append((fn_oid, 0, full_sig, 'FUNCTION'))

    for tg_foid, fn_name, tg_relid, rel_name, tg_name, nsp in triggers:
        full_sig = f"{tg_name} on {rel_name}"
        matched_filter = False
        if filter_names is None:
            matched_filter = True
        else:
            clean_tg = re.sub(r'^trg_\d+_', '', tg_name, flags=re.IGNORECASE).lower()
            for fn in filter_names:
                if fn in (fn_name.lower(), tg_name.lower(), clean_tg, rel_name.lower(), full_sig.lower()):
                    matched_filter = True
                    verified_targets.add(fn)
        if matched_filter:
            check_targets.append((tg_foid, tg_relid, full_sig, 'TRIGGER'))

    has_errors = False

    if filter_names is not None and len(filter_names) > 0:
        missing_targets = set(filter_names) - verified_targets
        for missing in sorted(missing_targets):
            runtime_issues.append((
                missing,
                f"[alvo não verificado] Objeto '{missing}' não foi encontrado no catálogo para verificação runtime."
            ))
            if require_checker:
                has_errors = True
                validation_results.append(
                    ValidationResult(
                        statement=SQLStatement(
                            ...
                            object_name=missing,
                            sql=f"-- plpgsql_check target not found: {missing}",
                            start_line=0
                        ),
                        success=False,
                        error_message=(
                            f"Alvo obrigatório '{missing}' não foi verificado pelo plpgsql_check "
                            f"(objeto não foi encontrado no catálogo do PostgreSQL)."
                        ),
                        pg_code="TARGET_NOT_VERIFIED"
                    )
                )
        ...
        if len(check_targets) == 0:
            if require_checker:
                has_errors = True
            if not any(iss[0] == "plpgsql_check" for iss in runtime_issues):
                runtime_issues.append((
                    "plpgsql_check",
                    "[verificação não realizada] Zero alvos verificados no catálogo, embora houvesse funções esperadas."
                ))
    ...
    elif filter_names is not None and len(filter_names) > 0 and (len(check_targets) == 0 or (set(filter_names) - verified_targets)):
        status = CHECK_ISSUES_FOUND
    elif runtime_issues:
        status = CHECK_ISSUES_FOUND
    else:
