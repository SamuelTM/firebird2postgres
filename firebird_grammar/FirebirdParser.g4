...
parser grammar FirebirdParser;

options {
    tokenVocab = FirebirdLexer;
}
...
sqlj_object_type
    : EXTERNAL NAME expression LANGUAGE JAVA USING (SQLDATA | CUSTOMDATUM)
    ;
...
unified_auditing
    :  AUDIT (
        ...
        | CONTEXT NAMESPACE firebird_namespace ATTRIBUTES attribute_name (',' attribute_name)* (
...
create_context
    : CREATE (OR REPLACE)? CONTEXT firebird_namespace USING (schema_object_name '.')? package_name (
...
period_definition
    :  PERIOD FOR column_name ('(' start_time_column ',' end_time_column ')')?
    ;
...
variable_declaration
    : (DECLARE? VARIABLE)? identifier CONSTANT? type_spec (NOT? NULL_)? default_value_part? ';'
    ;
...
assignment_statement
    : (general_element | bind_variable) (ASSIGN_OP | EQUALS_OP) expression
    ;
...
exit_statement
    : (EXIT | LEAVE) label_name? (WHEN condition)?
    ;
...
loop_statement
    ...
    | label_declaration? FOR select_statement DO statement
    | label_declaration? WHILE condition DO statement
    | label_declaration? FOR EXECUTE STATEMENT expression into_clause? DO statement
    ;
...
forall_statement
    : FORALL index_name IN bounds_clause (SAVE EXCEPTIONS)? (data_manipulation_language_statements | execute_statement)
    ;
...
call_statement
    : (CALL | EXECUTE PROCEDURE)? routine_name function_argument? ('.' routine_name function_argument?)* (
        INTO bind_variable
    )?
    ;
...
execute_statement
    : EXECUTE (IMMEDIATE | STATEMENT) expression (
...
select_statement
    : select_only_statement (for_update_clause | order_by_clause | offset_clause | fetch_clause)* into_clause?
    ;
...
dml_table_expression_clause
    ...
    | tableview_name ('(' expressions_? ')')? hierarchies_clause? sample_clause?
