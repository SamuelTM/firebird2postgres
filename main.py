...
def run_migration(migrator: DatabaseMigrator) -> bool:
    """
    Executes the end-to-end migration lifecycle from Firebird to PostgreSQL.
    Enforces source consistency verification BEFORE any export or destructive target operations.
    """
    # -------------------------------------------------------------
    # PRE-FLIGHT CHECK: Source consistency & frozen protection
...
if __name__ == '__main__':
    ...
    allow_live = '--allow-live-source' in sys.argv
    config = MigrationConfig(allow_live_source=allow_live) if allow_live else MigrationConfig()
    migrator = DatabaseMigrator(fb_connection, pg_connection, config=config)
